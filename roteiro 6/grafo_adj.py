...
class Grafo:
    ...
    def l_p(self, m):
        ...
        for i in range(len(self.N)):           #Ajusta laços e arestas paralelas
            for j in range(len(self.N)):
                if (i == j and m[j][i] > 0) or (m[j][i] > 1):
                    m[j][i] -= 1
        return m

    def warshall(self):

        matriz = self.M.copy()               #Algoritmo de Warshall
        matriz = self.l_p(matriz)            #Matriz sem laços e sem arestas paralelas
    ...
    def matriz_final(self):
        ...
        N_copia = self.N.copy()
        M_copia = self.warshall()

        Matriz_alcançabilidade = Grafo(N_copia, M_copia)
